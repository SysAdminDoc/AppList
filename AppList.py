...
def write_diff_report(diff: Dict[str, Any], filepath: Optional[str] = None) -> str:
    ...
    lines = []
    lines.append(f"AppList Diff Report")
...
<div class="header">
  ...
  <div class="meta">Machine: {html_mod.escape(hostname)} &middot; Generated: {html_mod.escape(timestamp)} &middot; Total: {len(apps)} applications</div>
</div>
<div class="stats">
  <div class="stat-card"><div class="stat-value">{len(apps)}</div><div class="stat-label">Total</div></div>
  {stat_cards}
</div>
<div class="toolbar">
  <input id="search" type="text" placeholder="Search applications..." oninput="filterTable()">
  <span class="count" id="count">Showing {len(apps)} of {len(apps)}</span>
</div>
...
function filterTable() {{
  ...
  document.getElementById('count').textContent = 'Showing ' + visible + ' of ' + {len(apps)};
}}
</script>
</body>
</html>"""
...
class AppList(ctk.CTk):
    ...
    def _create_toolbar(self):
        ...
        self.export_html_btn = SecondaryButton(
            export_frame,
            text="HTML",
            width=70,
        ...
        self.export_pip_btn = SecondaryButton(
            export_frame,
            text="pip",
            width=60,
            command=self._export_pip,
        )
        self.export_pip_btn.pack(side="left", padx=(0, 6))

        self.export_choco_btn = SecondaryButton(
            export_frame,
            text="Choco",
            width=72,
            command=self._export_choco,
        )
    ...
    def _export_html(self):
        """Export applications to a self-contained HTML dashboard."""
        if not self._ensure_exportable_rows():
            return
        ...
        filepath = filedialog.asksaveasfilename(
            ...
            initialfile=f"AppList_Dashboard_{timestamp}.html",
            title="Export as HTML Dashboard",
        )
        if not filepath:
            return
    ...
    def _export_pip(self):
        """Export pip packages as requirements.txt."""
        if not self._ensure_exportable_rows():
            return
        ...
        filepath = filedialog.asksaveasfilename(
            defaultextension=".txt",
            filetypes=[("Text Files", "*.txt"), ("All Files", "*.*")],
            initialfile=f"requirements_{timestamp}.txt",
            title="Export pip Requirements",
        )
        if not filepath:
            return
    ...
    def _export_choco(self):
        """Export Chocolatey packages as packages.config."""
        if not self._ensure_exportable_rows():
            return
        ...
        filepath = filedialog.asksaveasfilename(
            ...
            initialfile=f"packages_{timestamp}.config",
            title="Export Chocolatey Packages",
        )
        if not filepath:
            return
